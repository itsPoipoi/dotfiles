...
install_system_deps() {
  ...
  if ! yay -S --noconfirm --needed base-devel gcc make yazi ffmpeg 7zip jq poppler lsof fzf tumbler zoxide glow grc eza tree-sitter-cli pandoc-cli nwg-displays resvg imagemagick git ripgrep fd unzip neovim trash-cli bat fastfetch stow man-db less zsh; then
...
install_git_config() {
  ...
  if [[ "$skip_confirm" == "--yes" ]] || confirm_action "Set Git global config?"; then
    ...
    if confirm_action "Use default Git config?" "y"; then
      ...
      if [[ -n "$git_name" ]]; then git config --global user.name "$git_name" || print_error "Failed to set git user.name"; fi
      if [[ -n "$git_email" ]]; then git config --global user.email "$git_email" || print_error "Failed to set git user.email"; fi
    fi
  else
...
install_spicetify_setup() {
  if [[ ! -f "$HOME/.spicetify/spicetify" ]]; then
    if confirm_action "Setup Spicetify? (Spotify needs to be logged in first!)"; then
      ...
      \cp -f "$HOME/dotfiles/spicetify/keyboardShortcut.js" "$HOME/.spicetify/Extensions/"
